...
static void printIn(BST_Tree *T, BST_NodeT *pNode, FILE *fout)
{
    if(pNode == NIL) return;

    printIn(T, pNode->left, fout);
    T->printData(pNode->pData);
    fprintf(fout," appears %d times\n",pNode->count);
    printIn(T, pNode->right, fout);

}

void inBST(BST_Tree *T, FILE *fout)
{
    printIn(T, T->root, fout);
}
