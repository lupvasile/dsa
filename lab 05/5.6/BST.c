...
void insert_rec(BST_Tree *TM, BST_NodeT *pNode)
{
    if(pNode == NIL) return;
    insertBST(TM,pNode->pData);
    insert_rec(TM,pNode->left);
    insert_rec(TM,pNode->right);
}

void mergeTree(BST_Tree *TM, BST_Tree *T1, BST_Tree *T2)
{
    insert_rec(TM,T1->root);
    insert_rec(TM,T2->root);
}
